...
class UserDBManager:
    ...
    def create_message_history_table(self):
        """ایجاد جدول تاریخچه پیام‌ها برای ذخیره پیام‌های کاربران"""
        cur = self.conn.cursor()
        try:
            cur.execute("""
            ...
            logging.error(f"Error creating message_history table: {e}")
            return False

    def add_message_history(self, telegram_id, message_id, message):
        """افزودن پیام به تاریخچه پیام‌ها"""
        # اطمینان از وجود جدول
        self.create_message_history_table()
        
        cur = self.conn.cursor()
        try:
            cur.execute(
                "INSERT INTO message_history (telegram_id, message_id, message) VALUES (?, ?, ?)",
                (telegram_id, message_id, message))
            self.conn.commit()
            return True
        except Error as e:
    ...
    def find_message_history(self, **kwargs):
        """جستجوی پیام در تاریخچه پیام‌ها"""
        # اطمینان از وجود جدول
        self.create_message_history_table()
        
        cur = self.conn.cursor()
        try:
            query = "SELECT * FROM message_history WHERE "
            for key, value in kwargs.items():
                query += f"{key} = ? AND "
            query = query[:-5]  # حذف " AND " اضافی
            ...
            logging.error(f"Error finding message history: {e}")
            return None
