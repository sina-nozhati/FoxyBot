...
# User Configs Markup
def user_configs_markup(uuid):
    markup = InlineKeyboardMarkup()
    
    # افزودن دکمه‌ها برای انواع مختلف کانفیگ
    markup.add(
        InlineKeyboardButton(KEY_MARKUP['CONFIGS_SUB'], callback_data=f"user_configs_{uuid}_sub"),
        InlineKeyboardButton(KEY_MARKUP['CONFIGS_SUB_B64'], callback_data=f"user_configs_{uuid}_sub64")
    )
    markup.add(
        InlineKeyboardButton(KEY_MARKUP['CONFIGS_CLASH'], callback_data=f"user_configs_{uuid}_clash"),
        InlineKeyboardButton(KEY_MARKUP['CONFIGS_HIDDIFY'], callback_data=f"user_configs_{uuid}_hiddify")
    )
    markup.add(
        InlineKeyboardButton(KEY_MARKUP['CONFIGS_SING_BOX'], callback_data=f"user_configs_{uuid}_singbox"),
        InlineKeyboardButton(KEY_MARKUP['CONFIGS_FULL_SING_BOX'], callback_data=f"user_configs_{uuid}_full_singbox")
    )
    
    # دکمه بازگشت
    markup.add(InlineKeyboardButton(KEY_MARKUP['BACK'], callback_data=f"subscription_info_{uuid}"))
