...
import UserBot.templates as templates
...
# تابع برای بازیابی شناسه سرور براساس UUID
def get_server_id(uuid):
    """بازیابی شناسه سرور براساس UUID کاربر"""
    sub = USERS_DB.find_subscription_by_uuid(uuid)
    if sub:
        return sub['server_id']
    return None

# تابع برای ایجاد دکمه تبدیل به QR کد
def to_qr_markup(config_type, uuid):
    """ایجاد دکمه برای تبدیل پیکربندی به QR کد"""
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton(KEY_MARKUP['TO_QR'], callback_data=f"to_qr_{config_type}_{uuid}"))
    return markup
...
def get_user_configs(message, uuid):
    sub_url = URL = None
    server_id = get_server_id(uuid)
    if not server_id:
        bot.send_message(message.chat.id, MESSAGES['UNKNOWN_ERROR'],
                         reply_markup=main_menu_keyboard_markup())
        return
    ...
    msg_wait = bot.send_message(message.chat.id, MESSAGES['WAIT'])
    sub = utils.sub_parse(URL, uuid)  # استفاده از متد جدید sub_parse
    bot.delete_message(message.chat.id, msg_wait.message_id)
    
    if not sub:
        bot.send_message(message.chat.id, MESSAGES['USER_CONFIGS_NOT_FOUND'],
                         reply_markup=main_menu_keyboard_markup())
        return False
    
    return sub, URL

# ----------------------------------- Config callbacks Area -----------------------------------
# User Configs Handler
@bot.callback_query_handler(func=lambda call: "user_configs_" in call.data)
def user_configs_callback(call):
    bot.answer_callback_query(call.id)
    user_configs_data = call.data.split('_')
    if len(user_configs_data) < 3:
        bot.send_message(call.message.chat.id, MESSAGES['ERROR_INVALID_COMMAND'],
                         reply_markup=main_menu_keyboard_markup())
        return
    
    uuid = user_configs_data[2]
    config_type = 'sub'  # تنظیم نوع پیش‌فرض
    if len(user_configs_data) >= 4:
        config_type = user_configs_data[3]
    
    configs_result = get_user_configs(call.message, uuid)
    if not configs_result:
        return
    
    configs, URL = configs_result
    
    # بررسی نوع کانفیگ درخواستی و ارسال آن
    if config_type in configs:
        send_text = configs[config_type]
        
        # ارسال پاسخ متناسب با نوع کانفیگ
        if config_type == 'clash':
            bot.send_message(call.message.chat.id, f"{send_text}")
        elif config_type == 'hiddify':
            bot.send_message(call.message.chat.id, f"{send_text}")
        elif config_type == 'singbox':
            bot.send_message(call.message.chat.id, f"{send_text}")
        elif config_type == 'full_singbox':
            bot.send_message(call.message.chat.id, f"{send_text}")
        else:
            # برای سایر انواع (sub و sub64)
            text_msg = bot.send_message(call.message.chat.id, f"<code>{send_text}</code>",
                                     reply_markup=to_qr_markup(config_type, uuid))
            USERS_DB.add_message_history(telegram_id=call.message.chat.id, message_id=text_msg.message_id, message=send_text)
    else:
        bot.send_message(call.message.chat.id, MESSAGES['ERROR_CONFIG_NOT_FOUND'],
                         reply_markup=main_menu_keyboard_markup())

# Subscription Info Callback Handler
@bot.callback_query_handler(func=lambda call: call.data.startswith("subscription_info_"))
def subscription_info_callback(call: CallbackQuery):
    bot.answer_callback_query(call.id)
    uuid = call.data.replace("subscription_info_", "")
    sub_info = USERS_DB.find_subscription_by_uuid(uuid)
    if not sub_info:
    ...
    server = server[0]
    URL = server['url']
    user = api.find(URL, uuid=sub_info['uuid'])
    if not user:
        bot.delete_message(call.message.chat.id, msg_wait.message_id)
        bot.send_message(call.message.chat.id, MESSAGES['UNKNOWN_ERROR'],
                         reply_markup=main_menu_keyboard_markup())
    ...
    # محاسبه اطلاعات کاربر
    usage, days, last_connection = utils.calculate_user_remainings(user)
    hyperlink = False
    settings = utils.all_configs_settings()
    if settings and settings['show_hiddify_link'] == 1:
        hyperlink = True
    
    # ایجاد پیام وضعیت اشتراک
    template = templates.user_info_template(
        sub_info['name'], server['title'], usage, days, sub_info['uuid'],
        server['url'], hyperlink, last_connection
    )
    ...
    # تنظیم دکمه‌های منو
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton(KEY_MARKUP['CONFIGS_LIST'], callback_data=f"show_configs_{uuid}"))
    
    # افزودن دکمه جداسازی اشتراک
    markup.add(InlineKeyboardButton(KEY_MARKUP['UNLINK_SUBSCRIPTION'], callback_data=f"unlink_subscription_{uuid}"))
    
    # افزودن دکمه بازگشت
    markup.add(InlineKeyboardButton(KEY_MARKUP['BACK'], callback_data="back_to_main"))
    
    bot.edit_message_text(template, call.message.chat.id, call.message.message_id, reply_markup=markup)

# نمایش لیست کانفیگ‌ها
@bot.callback_query_handler(func=lambda call: call.data.startswith("show_configs_"))
def show_configs_callback(call: CallbackQuery):
    bot.answer_callback_query(call.id)
    uuid = call.data.replace("show_configs_", "")
    
    # ارسال پیام لیست کانفیگ‌ها
    bot.edit_message_text(MESSAGES['USER_CONFIGS_LIST'], call.message.chat.id, call.message.message_id,
                         reply_markup=user_configs_markup(uuid))

# پردازش دکمه QR کد
@bot.callback_query_handler(func=lambda call: call.data.startswith("to_qr_"))
def to_qr_callback(call: CallbackQuery):
    bot.answer_callback_query(call.id)
    try:
        # استخراج نوع پیکربندی و UUID
        parts = call.data.split('_')
        config_type = parts[2]
        uuid = parts[3]
        
        # دریافت پیام متنی قبلی که باید به QR تبدیل شود
        message_history = USERS_DB.find_message_history(message_id=call.message.message_id)
        if not message_history:
            bot.edit_message_text(MESSAGES['ERROR_FAILED_GENERATE_QR'], call.message.chat.id, call.message.message_id)
            return
            
        # محتوای پیام که باید به QR تبدیل شود
        message_content = message_history[0]['message']
        
        # ایجاد و ارسال تصویر QR کد
        qr_image = utils.create_qr_code(message_content)
        bot.send_photo(call.message.chat.id, qr_image, caption=MESSAGES['QR_CODE_CAPTION'])
    except Exception as e:
        logging.error(f"QR generation error: {e}")
        bot.send_message(call.message.chat.id, MESSAGES['ERROR_FAILED_GENERATE_QR'])
