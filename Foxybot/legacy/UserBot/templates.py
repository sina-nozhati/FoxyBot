...
def user_info_template(name, server_title, usage, days, uuid, server_url=None, hyperlink=False, last_connection=None, header=""):
    """قالب نمایش اطلاعات اشتراک کاربر"""
    if hyperlink and server_url:
        user_name = f"<a href='{server_url}/{uuid}/'> {name} </a>"
    else:
        user_name = name
...
{MESSAGES['SERVER']} {server_title}
{MESSAGES['INFO_USAGE']} {usage['current_usage_GB']} {MESSAGES['OF']} {usage['usage_limit_GB']} {MESSAGES['GB']}
{MESSAGES['INFO_REMAINING_DAYS']} {days} {MESSAGES['DAY_EXPIRE']}
{MESSAGES['INFO_ID']} <code>{uuid}</code>
"""

    # اضافه کردن آخرین اتصال اگر موجود باشد
    if last_connection:
        template += f"آخرین اتصال: {last_connection}\n"
