...
# Convert users list from API to dict format
def users_to_dict(users_dict):
    """
    تبدیل ساختار دریافتی از API به فرمت دیکشنری برای استفاده در برنامه
    این تابع برای سازگاری با API جدید تطبیق داده شده است
    """
    logging.info(f"Convert users list from API to dict format - {len(users_dict) if users_dict else 0} users")
    if not users_dict:
        return []
    res = []
    for user in users_dict:
        if isinstance(user, dict):
            ...
            user_data = {
                ...
                "start_date": user.get('start_date', '2023-01-01'),
...
# Process user dict to add remaining days, usage, etc.
def dict_process(url, users_dict, sub_id=None, server_id=None):
    logging.info(f"Process user dict to add remaining days, usage, etc. - {len(users_dict)} users")
    if not users_dict:
        return False
    
    proxy_path, api_key = api.parse_panel_url(url)
    ...
    res = []
    for user in users_dict:
        # If link is empty, create one
        if 'link' not in user:
            user['link'] = f"{base_url}/{proxy_path}/{user['uuid']}/"
        
        # ساخت دیکشنری برای داده‌های مصرف و روزهای باقیمانده
        usage = {
            "remaining_usage_GB": calculate_remaining_usage(user['usage_limit_GB'], user['current_usage_GB']),
            "percent_usage": int((user['current_usage_GB'] / user['usage_limit_GB']) * 100) if user['usage_limit_GB'] != 0 else 0
        }
        ...
        # Add to user dict
        user['remaining_day'] = remaining_day
        user['usage'] = usage
        
        # اضافه کردن اطلاعات لینک کانفیگ
        sub = sub_links(user['uuid'], url)
        if sub:
            user['sub_links'] = sub
        ...
        if 'start_date' in user:
            user['start_date'] = user['start_date'].split(' ')[0]
        if 'last_reset_time' in user:
            user['last_reset_time'] = user['last_reset_time'].split(' ')[0]
        
        # کاربر فرآیند شده را به نتیجه اضافه کن
        res.append(user)
...
def sub_links(uuid, url= None):
    ...
    # آدرس‌های جدید مطابق با API 2.2.0
    ...
    # آدرس‌های جدید برای auto و singbox
...
# Parse sub links
def sub_parse(url, uuid):
    """
    تجزیه لینک اشتراک و استخراج لینک‌های پروتکل‌های مختلف
    """
    try:
        proxy_path, api_key = api.parse_panel_url(url)
        if not proxy_path or not api_key:
        ...
        # دریافت کانفیگ‌های کاربر با استفاده از API جدید
        configs = api.get_user_configs(url, uuid)
        if not configs:
            logging.error("No configs returned from API")
            return None
        ...
        if isinstance(configs, dict):
            # در API جدید، کانفیگ‌های مختلف در کلیدهای مختلف قرار دارند
            # استخراج لینک‌های موردنیاز
            if 'subscription_url' in configs:
                result['sub'] = configs['subscription_url']
            
            if 'subscription_url_encoded' in configs:
                result['sub64'] = configs['subscription_url_encoded']
                
            if 'clash_configs' in configs:
                result['clash'] = configs['clash_configs']
                
            if 'hiddify_configs' in configs:
                result['hiddify'] = configs['hiddify_configs']
                
            if 'singbox_configs' in configs:
                result['singbox'] = configs['singbox_configs']
                
            if 'full_singbox_configs' in configs:
                result['full_singbox'] = configs['full_singbox_configs']
        else:
            # ساختار قدیمی یا غیرمنتظره
            logging.error("Unexpected API response structure")
            return None
        
        return result
    except Exception as e:
        logging.error("sub_parse error: %s" % e)
        return None


# Backup panel
def backup_panel(url):
    logging.info(f"Backup panel: {url}")
    # استخراج proxy_path و api_key
    proxy_path, api_key = api.parse_panel_url(url)
    if not proxy_path or not api_key:
        return False
    ...
    # آدرس API پشتیبان‌گیری در نسخه جدید
    backup_url = f"{base_url}/{proxy_path}/api/v2/admin/backup/get_backup/"
    
    headers = {"Hiddify-API-Key": api_key}
    response = requests.get(backup_url, headers=headers)
    
    if response.status_code != 200:
        logging.error(f"Error in backup panel: {response.text}")
        return False
...
def search_user_by_uuid(url, uuid):
    # استفاده مستقیم از تابع find جدید به جای select
    user = api.find(url, uuid)
    if user:
        return [user]
    return False
...
def find_subscription_by_uuid(uuid):
    """
    پیدا کردن اشتراک کاربر بر اساس UUID
    این تابع ابتدا در subscription های سفارش و سپس در subscription های بدون سفارش جستجو می‌کند
    """
    # جستجو در سفارش‌ها
    order_users = USERS_DB.find_order_subscription(uuid=uuid)
    if order_users:
        return order_users[0]
        
    # جستجو در اشتراک‌های بدون سفارش
    non_order_users = USERS_DB.find_non_order_subscription(uuid=uuid)
    if non_order_users:
        return non_order_users[0]
        
    return None

# اضافه کردن این تابع به USERS_DB
USERS_DB.find_subscription_by_uuid = find_subscription_by_uuid

# محاسبه اطلاعات باقی‌مانده کاربر
def calculate_user_remainings(user):
    """
    محاسبه اطلاعات باقی‌مانده کاربر شامل مصرف، روزهای باقی‌مانده و آخرین اتصال
    """
    try:
        # تبدیل به دیکشنری اگر لازم است
        ...
        usage = {
            ...
            'percent_usage': int((user.get('current_usage_GB', 0) / user.get('usage_limit_GB', 0)) * 100) if user.get('usage_limit_GB', 0) != 0 else 0
        }
        
        # محاسبه روزهای باقی‌مانده
        days = calculate_remaining_days(user.get('start_date', None), user.get('package_days', 0))
        
        # محاسبه آخرین اتصال
        last_connection = None
        if 'last_online' in user and user['last_online']:
            last_connection = calculate_remaining_last_online(user['last_online'])
        else:
            from AdminBot.content import MESSAGES
            last_connection = MESSAGES['NEVER']
            
        return usage, days, last_connection
    except Exception as e:
        logging.error(f"Error calculating user remainings: {e}")
        return {}, 0, "خطا"

# Create QR code
def create_qr_code(text):
    """تبدیل متن به تصویر کد QR"""
    try:
        import qrcode
        import io
