...
if [ ! -d "$install_dir/venv" ]; then
  display_error_and_exit "Virtual environment not found. Please run install.sh first."
fi
...
> $install_dir/bot.log
cd $install_dir && source venv/bin/activate && nohup python3 hiddifyTelegramBot.py >> bot.log 2>&1 &
